...
order_date = ['16-mar-20','05-mar-21','15-MAR-22','24-JAN-22','14-MAR-22', '14-jan-21' ]
symbol = ["AMZN",'AAPL','TSLA','MSFT','PTON']
withdrawal = ['Amount you wish to withdrawal']
share_price = ['Share price you purchased at']

page_title = 'Tax Harvesting App'
page_icon = ':tada:'
layout = 'centered'
...
with st.container():
    ...
    st.title('Tax Harvesting for portfolios')
    st.write('This app will display your portfolio, you will choose the number of shares you wish to sell from a certain ticker and we will determine the best way for you to take liquidity from your portfolio while paying the least percentage in taxes.')
    st.write('---')
...
st.write(portfolio)
st.header(f'Withdrawal in USD')
st.write('Below enter the date in which you bought the stock then enter the ticker you would like to withdrawal from, as well as the number of shares you would like to sell.')
with st.form('Amount you wish to withdrawal',clear_on_submit=True):
    col1, col2, col3,col4 = st.columns(4)
    col1.selectbox('Select order_date (YYYY/MM/DD)', order_date, key=order_date)
    col2.selectbox("select ticker:", symbol, key=symbol)
    col3.number_input("Select amount of shares you wish to sell:", min_value = 0, format='%i',step=1,key=withdrawal)
    col4.number_input("Share price you purchased at:", min_value = 0, format='%i',step=1,key=share_price)
    '---'
    submitted = st.form_submit_button("Submit Withdrawal Request")
    if submitted:
        period = str(st.session_state['order_date']) + '_' + str(st.session_state[symbol])+ '_' + str(st.session_state['withdrawal'])
        order_date = {order_date: st.session_state[order_date] for order_date in order_date}
        symbol = {symbol: st.session_state[symbol] for symbol in symbol}
        withdrawal = {withdrawal: st.session_state[withdrawal] for withdrawal in withdrawal}

        # TODO: insert values into database
        st.write(f'order_date:{order_date}')
        st.write(f'symbol:{symbol}')
        st.write(f'withdrawal:{withdrawal}')
...
with st.form('saved_periods'):
    ...
    period = st.selectbox('Select Period:',['16-mar-20','05-mar-21','15-MAR-22','24-JAN-22','14-MAR-22', '14-jan-21'])
